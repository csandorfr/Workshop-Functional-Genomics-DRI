...
## Sept up your work environnement

To evaluate a  the phenotypic linkage network you must:

1) Install some R/Bioconductor package
2) Indicate a directory where you will download PLN directory (require ~10Gb on your disk)
3) Indicate a list of genes (format gene symbol)

### The following package must to be installed

You must install the following packages and load them.
...
You must indicate a work directory where the PLN file will be downloaded
...
You must specify where is your list of gene
...
Here the parameters requiry to evaluate the functional links between your list of genes by using PLN
...
We used the final linkage network build with the following dataset
...
We run a Rscript to evaluate and represent the functional relationship between gene set.
There are two part displayed in two different plot:

Plot1: Histogramm

We evaluates whether a given set of genes demonstrate unusually similar functionality. It examines the extent to which those genes cluster within the PLN. For this, it compares the sum of weighted links observed between
these genes as compared to an equal number of random genes and computes an empirical p-value. As the
...
It extracts the most contributing resources, among the integrated datasets to build a PLN, for any functional associations found; (3) represents the extracted subnetwork with links colored by the most contributing resource.
